...
#[test]
fn platform_capability_leaves_cross_every_executable_backend() {
    let source = library("std/platform.mncs");
    let corpus = example("execution/platform-capability-corpus.json");
    for backend in [
        ...
        let output = Command::new(env!("CARGO_BIN_EXE_mncs"))
            ...
            .output()
            .expect("run platform capability experiment");
