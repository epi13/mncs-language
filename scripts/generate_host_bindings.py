...
def rust_descriptor(contract: Any) -> str:
    """Describe a contract without erasing Rust's signedness boundary."""
    ...
    if "boolean" in value:
        return "bool"
    if "integer" in value:
        return "int" if value["integer"].get("signed", True) else "uint"
    if "float" in value:
        return "float"
    if "byte" in value:
        return "byte"
    ...
    if "sequence" in value:
        ...
        element = sequence.get("element")
        bound = sequence.get("bound", {}).get("exact")
        return f"sequence:{rust_descriptor(element)}:{bound if bound is not None else ''}"
    if "vector" in value:
        vector = value["vector"]
        return f"vector:{rust_descriptor(vector.get('element'))}:{vector.get('lanes', '')}"
    if "mask" in value:
        return f"mask:{value['mask'].get('lanes', '')}"
    if "scalar" in value:
        semantic = value["scalar"].get("semantic_type")
        if semantic == "bool":
            return "bool"
        if isinstance(semantic, dict) and "integer" in semantic:
            return "int" if semantic["integer"].get("signed", True) else "uint"
        if isinstance(semantic, dict) and "float" in semantic:
            return "float"
        return "scalar"
    return "unknown"
...
def rust_type(contract: Any) -> str:
    descriptor = rust_descriptor(contract)
...
def rust_encode_expression(descriptor: str, expression: str) -> str:
    ...
    if descriptor == "uint":
        return f"json!({{\"integer\": {{\"value\": {expression}, \"type\": {{\"bits\": 64, \"signed\": false}}}}}})"
    if descriptor == "byte":
        return f"json!({{\"byte\": {{\"value\": {expression}}}}})"
    if descriptor == "float":
...
def rust_decode_result_expression(descriptor: str, expression: str) -> str:
    ...
    if descriptor == "uint":
        return f"decode_u64({expression})"
    if descriptor == "byte":
        return f"decode_u8({expression})"
    if descriptor == "float":
...
def rust_binding(abi: dict[str, Any]) -> str:
    ...
    out = [
        ...
        "    let integer = decode_object(value, \"integer\")?.get(\"integer\").and_then(Value::as_object).ok_or_else(|| EmbedError::new(\"binding_decode\", \"typed integer is malformed\"))?;",
        "    let type_info = integer.get(\"type\").and_then(Value::as_object).ok_or_else(|| EmbedError::new(\"binding_decode\", \"typed integer has no type\"))?;",
        "    if type_info.get(\"bits\").and_then(Value::as_u64) != Some(64) || type_info.get(\"signed\").and_then(Value::as_bool) != Some(false) { return Err(EmbedError::new(\"binding_decode\", \"typed integer is not u64\")); }",
        "    integer.get(\"value\").and_then(Value::as_u64).ok_or_else(|| EmbedError::new(\"binding_decode\", \"typed u64 is malformed\"))",
        "}",
        "fn decode_u8(value: &Value) -> Result<u8, EmbedError> {",
        "    decode_object(value, \"byte\")?.get(\"byte\").and_then(Value::as_object).and_then(|v| v.get(\"value\")).and_then(Value::as_u64).and_then(|v| u8::try_from(v).ok()).ok_or_else(|| EmbedError::new(\"binding_decode\", \"typed byte is malformed\"))",
        "}",
