...
examples/source/proof-demo-no-overflow.mncs
    fn add_small() -> (result: i64) { let total: i64 = 2 + 3; return total; }
        |
        | elaborate + generate_obligations
        v
O = mncs:0.2:obligation:body:integer-overflow:9db4fac4…
    status UNKNOWN, method symbolic-Checked
    subject mncs:0.2:operation:examples.proof_demo::add_small::entry::v2
        |
        | lower_to_ssa  (the SAME identity rides along)
        v
SSA Integer { add, i64 signed, Checked } fed by constants 2 and 3,
...
binding: proof mncs:proof:<sha256>, kernel mncs:proof-kernel:0.1,
        ...
        +---> integer_no_overflow_promise (real lowering) grants NoOverflow;
