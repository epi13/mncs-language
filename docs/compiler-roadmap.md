...
- **C3 backend-plural prototype implemented locally:** `mncs-codegen::BackendAdapter` realizes selected SSA through identity-bound capability manifests, target/configuration contracts, plans, typed artifacts, and adapter-owned execution. Portable WASM MVP and canonical research bytecode compile the same exact selected SSA without changing compiler sequencing. Unevidenced targets still produce `UNKNOWN` and no backend artifact. Body/SSA/backend comparison is empirical bounded agreement over a frozen corpus. Native backends remain unimplemented.
...
The stage audit in RFC 0039 makes the complete logical pipeline machine-readable through `mncs compiler-architecture`. It reports the Profile 0.1/0.2 frontend and backend-adapter realization as experimental rather than treating the semantic JSON bootstrap path or WASM as the language definition. Compiler study results expose per-stage fingerprints, pass execution records, backend/plan/artifact identities, and unresolved obligations for Forge comparison. `mncs:language:experiment-result:0.1` additionally binds source, corpus, validators, realization request, exact artifacts, and case observations.

Source Profile 0.1 remains the identity-function grammar. Source Profile 0.2 elaborates lexical bindings, named `requires`/`ensures`/`assumes` clauses, declared capabilities, capability-authorized effects, `if`/`fail`, exact `bool`/integer types, and checked integer expressions into the existing semantic contract model. Ambiguous or unresolved bindings, duplicate functions, type mismatches, and unreachable statements are explicit errors. The elaborator does not invent capability authority.
