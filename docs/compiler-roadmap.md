...
- **C3 prototype implemented locally:** `mncs-codegen` lowers selected SSA to a portable WASM MVP artifact when the target contract carries explicit layout, ABI, integer, and trap facts. Unevidenced targets still produce `UNKNOWN` and no backend artifact. Research execution is the embedded interpreter, not a host wasm CLI. Body/SSA/backend comparison is empirical bounded agreement over a frozen corpus. Native backends remain unimplemented.
...
The stage audit in RFC 0039 makes the complete logical pipeline machine-readable through `mncs compiler-architecture`. It reports the Profile 0.1/0.2 frontend as experimental and portable executable generation as experimental rather than treating the semantic JSON bootstrap path as the only frontend. Compiler study results expose a language-owned observation contract with per-stage fingerprints, pass execution records, and unresolved obligations for Forge comparison.

Source Profile 0.1 remains the identity-function grammar. Source Profile 0.2 elaborates lexical bindings, named `requires`/`ensures`/`assumes` clauses, declared capabilities, capability-authorized effects, `if`/`fail`, and checked integer expressions into the existing semantic contract model. Ambiguous bindings are explicit errors. The elaborator does not invent capability authority.
