...
- **Independent reference** (`crates/mncs-model/src/proof_dep.rs`, ~2000
  ...
  disputes.
- **Differential agreement**: exact, on all 20 curated cases (closed and
  flagship theorems, beta/defeq probes, canonical assumption sets,
  dependent elim, four adversarial FAIL cases, one UNKNOWN abstention).
  The Rust suite additionally covers sealed artifacts (`DepArtifact`,
  `mncs:proof-dep:<sha256>`) and corroboration policy
  (`corroborate_proof` / `DepCorroboration`, including dispute quarantine).
- **Authority split (hardening pass)**: the Rust checker is corroboration
  only — it recomputes a diagnostic verdict, compares it with the
...
- Curated corpus: 20 cases in `examples/execution/proof-dep-corpus.json`,
  ...
  `examples/execution/proof-dep-admission-corpus.json` (2 seal, 5
  authorize, 3 reuse, 11 mutation-per-rejection-leg), both generated by
  `scripts/gen_proof_dep_corpus.py` (generation is untrusted by design;
  expectations are theory-derived, never recorded from runs).
- Research-bytecode execution: 20/20 main expectations met and 21/21
  admission expectations met, zero traps; the flagship checks in ~754k
  steps, the dearest main case (`flagship-set-empty`) in ~1059k steps
  against a 2000k step budget, the dearest admission cases
  (`admit-reuse-flagship` ~1823k, `admit-seal-flagship` ~1813k steps)
  against a 4000k step budget.
- Assumption representation: the lossy base-33 `assumptions_code` remains
  as a diagnostic/backward-compatibility projection only (still
  differentially pinned by two corpus cases). Authority uses the canonical
  `AssumptionSet` (exact used-hypothesis triples in buffer order; the
  empty set is explicit; non-PASS yields `valid == false`), compared
  exactly on both sides.
- Five-backend agreement: table below (re-verified in the hardening pass).

## Five-backend agreement (tranche 0.2 corpora, hardening pass)

Executed `examples/execution/proof-dep-corpus.json` (20 cases) and
`examples/execution/proof-dep-admission-corpus.json` (21 cases) on all
five evidence backends via `mncs experiment run`; every backend process
exited 0 and every case output matched its pinned expectation byte-for-byte
...
| corpus   | backend                   | exit | cases met | divergences | traps |
|----------|---------------------------|------|-----------|-------------|-------|
| main     | mncs-research-bytecode    | 0    | 20/20     | 0           | 0     |
| main     | mncs-portable-wasm-mvp    | 0    | 20/20     | 0           | 0     |
| main     | mncs-c11                  | 0    | 20/20     | 0           | 0     |
| main     | mncs-llvm-ir              | 0    | 20/20     | 0           | 0     |
| main     | mncs-cranelift            | 0    | 20/20     | 0           | 0     |
| admission| mncs-research-bytecode    | 0    | 21/21     | 0           | 0     |
| admission| mncs-portable-wasm-mvp    | 0    | 21/21     | 0           | 0     |
| admission| mncs-c11                  | 0    | 21/21     | 0           | 0     |
| admission| mncs-llvm-ir              | 0    | 21/21     | 0           | 0     |
| admission| mncs-cranelift            | 0    | 21/21     | 0           | 0     |

205 of 205 cases met across both corpora and all five backends. The
native-backend rows initially failed and were fixed, not waived (see
"Native-backend findings" below): as first measured, C11 segfaulted both
flagship admission cases (4 MiB arena exhaustion), LLVM additionally
crashed every `binding_reusable` case (truncated nested-record offsets),
and Cranelift silently returned `false` for flagship reuse (dropped
writes/zero reads past the arena cap). Each failure was contained (never
a false PASS) and each is now a passing case above.

## Native-backend findings (hardening pass)

Two root-cause defects, both in code this tranche added or stressed, both
fixed minimally with the corpora as oracle:

- **Native arena exhaustion (C11/LLVM crash).** All native realizations
  shared a 4 MiB bump arena. Flagship-scale admission
  (`seal_binding`/`binding_reusable`: kernel check plus canonical
  assumption-set recomputation plus the `ProofBinding` return record)
  peaks at 6,020,616 measured bytes (`admit-reuse-flagship`), so the
  child wrote past the static array and died (SIGSEGV, empty stderr).
  Fix: one shared constant
  (`crates/mncs-codegen/src/support.rs::NATIVE_ARENA_BYTES`, 16 MiB ≈
  2.6× measured headroom) consumed by the C11, LLVM IR, AOT cell-runtime,
  and Cranelift JIT emitters, so module and driver can never disagree on
  the size again.
- **Nested-record slot truncation (LLVM crash).** Qualified field
  spellings (`dep.AssumptionSet`) canonicalize to full record identities,
  but the composite-layout width table matched short names only, so a
  nested-record field collapsed to a 32-bit slot. C11 survived by
  zero-extension luck; LLVM stored 4 bytes and reloaded 8, forging wild
  arena offsets out of stack garbage (SIGSEGV in `assumption_set_equal`).
  Fix: match record/finite identities as well as names in
  `slot_width` (`crates/mncs-codegen/src/composite.rs`).
- **Silent out-of-bounds host access (Cranelift wrong answer).**
  Exhaustion made the JIT host calls drop stores and read zeros instead
  of failing, so flagship reuse returned a divergent `false`. Fix: a
  sticky out-of-bounds flag in the four host slot functions; the execute
  path reports `RuntimeFailure` while it is set
  (`crates/mncs-codegen/src/cranelift_backend.rs`).

Fail-closed notes: every one of these failed loudly at the observation
layer (child death → `invalid_request`, never PASS), and the Cranelift
wrong-answer direction was deny-reuse (`false`), never a forged grant.
The bump allocators still carry no in-band overflow guard past 16 MiB;
that residual is recorded under remaining risks, with the
runner-loud guarantee unchanged.

The independent Rust reference (`cargo test -p mncs-model proof_dep`,
23 tests including the flagship, the canonical assumption sets, the
corroboration policy table, the sealed artifact round-trip, and a
corpus-agreement test over the same 20-case file) agrees with every pinned
expectation, so MNCS-native and reference verdicts coincide on all 20 main
cases (the admission corpus is MNCS-authoritative by design and is covered
on the Rust side through genuine MNCS execution plus corroboration in
`crates/mncs-compiler` admission tests and CLI ingestion tests). The
harness-level report status reads `UNKNOWN` on every run because compiling
the kernel module itself retains 969 unresolved compiler obligations (660
integer-overflow bodies plus sequence-bounds, iteration-cost, and
realization-branchless kinds — the same standing phenomenon as tranche
0.1's retained obligations, and unrelated to the per-case proof verdicts);
every per-case expectation is met and every backend process exited 0.

## Remaining gaps after tranche 0.2 (hardening-pass status)

G4 (proof transport through every compiler stage): the minimum real path
now exists — file artifact → `mncs compile --proof` ingestion → MNCS
admission execution → corroboration → `ProofRelationship` → validated HIR
attachment → validated SSA attachment → MNCS-authorized proof-bearing
lowering evidence (`crates/mncs-cli/tests/proof_dep_admission.rs`,
7 tests). Stale/mutated relationships are refused at every stage.
Criterion 0007-C14 is satisfied for this integration scope; full
proof-discharge (proof proposition entailing the obligation) and
backend/codegen-stage transport remain future work and are never claimed.

G5 (file-based proof ingestion): closed by `mncs compile --proof
<artifact.json> [--proof-operation <id>]` with the end-to-end test above
(file → toolchain → MNCS PASS → binding → consumption → evidence →
mutation rejected).

G6 (erasure): out of scope for this pass, stays open (excused).

Heterogeneous worker evidence (C16): one genuine remote worker plus
same-host mediation (see
`docs/development-evidence/rfc0007-tranche02-fabric-remote-2026-09.md`):
`fabric-worker-01` (Fedora 43 x86_64) executed both corpora on the
embedded bytecode and WASM backends via a content-addressed Fabric job —
82 of 82 expectations met, outcome PASS with receipts. The Windows worker
answered a platform probe but has no runnable MNCS artifact, so it
checked no case. C16 stays partial: single remote worker, no Windows
verdict.